...
class CFlir:
    ...
    def raw2temp(
        ...
        raw_refl2 = (
            PR1 / (PR2 * (exp(PB / (RTemp + 273.15)) - PF)) - PO
    ...
    def generate_colorbar(self, min_temp=None, max_temp=None, cmap=cv.COLORMAP_JET, height=None):
        ...
        append_img[
            append_img.shape[0] // 2
            - cb_color.shape[0] // 2 : append_img.shape[0] // 2
            - (cb_color.shape[0] // 2)
            + cb_color.shape[0],
            10 : 10 + cb_color.shape[1],
        ] = cb_color
        cv.putText(
            append_img,
            str(min_temp),
            (5, append_img.shape[0] // 2 - (cb_color.shape[0] // 2) + cb_color.shape[0] + 30),
        ...
        cv.putText(
            append_img,
            str(max_temp),
            (5, append_img.shape[0] // 2 - cb_color.shape[0] // 2 - 20),
            cv.FONT_HERSHEY_PLAIN,
            1,
            (0, 0, 255),
            1,
    ...
    def extract_temperatures(self):
        ...
        if os.name == "nt":
            # Windows
            meta_json = sp.Popen(
                f'exiftool.exe "{self.image_path}" -Emissivity -ObjectDistance -AtmosphericTemperature -ReflectedApparentTemperature -IRWindowTemperature -IRWindowTransmission -RelativeHumidity -PlanckR1 -PlanckB -PlanckF -PlanckO -PlanckR2 -j',
                shell=True,
                stdout=sp.PIPE,
            ).communicate()[0]
        else:
            # Linux
            meta_json = sp.Popen(
                f'exiftool "{self.image_path}" -Emissivity -ObjectDistance -AtmosphericTemperature -ReflectedApparentTemperature -IRWindowTemperature -IRWindowTransmission -RelativeHumidity -PlanckR1 -PlanckB -PlanckF -PlanckO -PlanckR2 -j',
                shell=True,
                stdout=sp.PIPE,
            ).communicate()[0]
        ...
        raw2tempfunc = lambda x: CFlir.raw2temp(
            x,
            E=meta["Emissivity"],
            OD=CFlir.parse_length(meta["ObjectDistance"]),
            RTemp=CFlir.parse_temp(meta["ReflectedApparentTemperature"]),
            ATemp=CFlir.parse_temp(meta["AtmosphericTemperature"]),
            IRWTemp=CFlir.parse_temp(meta["IRWindowTemperature"]),
            IRT=meta["IRWindowTransmission"],
            RH=CFlir.parse_percent(meta["RelativeHumidity"]),
            PR1=meta["PlanckR1"],
            PB=meta["PlanckB"],
            PF=meta["PlanckF"],
            PO=meta["PlanckO"],
    ...
    def get_measurement_areas_values(self, image, thermal_np, raw_thermal_np, is_rect=False):
        ...
        for i in range(0, len(self.measurement_contours)):
            raw_vals, thermal_vals, indices = CFlir.get_roi(
                image, thermal_np, raw_thermal_np, self.measurement_contours, i
            )
        ...
        for i in range(0, len(self.measurement_rects)):
            measurement_areas_thermal_values.append(
                CFlir.get_roi(
                    ...
                    self.measurement_contours,
                    i,
                    area_rect=self.measurement_rects[i],
                )[1]
            )
...
class CSeqVideo:
    ...
    def split_thermal(self, input_video, output_folder=None, path_to_base_thermal_class_folder="."):
        ...
        for line in tqdm(self.split_by_marker(open(inputname, "rb"), marker=pat)):
            ...
            Thread(
                target=get_thermal_image_from_file, kwargs={"thermal_class": CFlir, "thermal_input": outname}
            ).start()
            idx = idx + 1
            if idx % 100000 == 0:
